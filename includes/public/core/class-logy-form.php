	...
	function get_form_elements( $form = null ) {
		...
		case 'login':
			...
			$fields[] = array(
				'item' 	=> 'input',
				'icon'	=> 'user',
				'label'	=> __( 'Username or Email', 'logy' ),
				'id'	=> 'user_login',
				'name'	=> 'log',
				'type'	=> 'text'
			);
		...
		case 'register':
			...
			if ( is_registration_incomplete() ) {
				...
				if ( isset( $required_fields['email'] ) ) {
					$errors[] = sprintf( __( "- %s didn't provide us with your email.", 'logy' ), $required_fields['provider'] );
				}

				if ( isset( $required_fields['user_login'] ) ) {
					$errors[] = __( "- We couldn't get your username or its already exist.", 'logy' );
				}
				...
				if ( ! isset( $_GET['register-errors'] ) ) {
					$fields[] = array(
						'item' 	=> 'note',
						'note'	=> sprintf( __( "<strong>Note:</strong> We coudn't get the informations below : <br> %s", 'logy' ), $erros_msg ) 
					);
				}
			...
			$fields[] = array(
				'item' 	=> 'note',
				'note'	=> __( '<strong>Note:</strong> We will email you a link you can follow to set your account password.', 'logy' )
			);
			...
			if ( 'on' == logy_options( 'logy_show_terms_privacy_note' ) ) {
				...
				$fields[] = array(
					...
					'note'  => sprintf( __( 'By creating an account you agree to our <a href="%1s">Terms and Conditions</a> and our <a href="%2s" >Privacy Policy</a>.', 'logy' ), $terms_url, $privacy_url )
				);
			}
		...
		case 'lost_password':
			...
			if ( isset( $_GET['action'] ) && 'rp' == $_GET['action'] ) {
				...
				$fields[] = array(
					'item' 	=> 'input',
					'icon'	=> 'lock',
					'label'	=> __( 'repeat new password', 'logy' ),
					'id'	=> 'pass2',
					'name'	=> 'pass2',
					'type'	=> 'password'
				);
				...
				$fields[] = array(
					'item' 	=> 'note',
					'note'	=> __( "Enter your email address and we'll send you a link you can use to pick a new password.", 'logy' )
				);
	...
	function get_form_messages( $attrs ) {
		...
		<?php if ( isset( $attrs['registered'] ) && $attrs['registered'] ) : ?>
			<div class="logy-form-message logy-success-msg">
				<p>
					...
					<?php _e( 'You have successfully registered. We have emailed your password to the email address you entered.', 'logy' ); ?>
				</p>
			</div>
		<?php endif; ?>
		...
		<?php if ( isset( $attrs['logged_out'] ) && $attrs['logged_out'] ) : ?>
			<div class="logy-form-message logy-info-msg">
				<p>
					<?php _e( '<strong>You have signed out!</strong> Would you like to sign in again?', 'logy' ); ?>
				</p>
			</div>
		<?php endif; ?>
		...
		<?php if ( isset( $attrs['lost_password_sent'] ) && $attrs['lost_password_sent'] ) : ?>
			<div class="logy-form-message logy-info-msg">
				<p>
					<?php _e( 'Check your email for a link to reset your password.', 'logy' ); ?>
				</p>
			</div>
		<?php endif; ?>
		...
		<?php if ( isset( $attrs['password_updated'] ) && $attrs['password_updated'] ) : ?>
			<div class="logy-form-message logy-success-msg">
				<p>
				<strong><?php _e( 'done!' , 'logy' ); ?></strong>
					<?php _e( 'Your password has been changed. you can sign into your account now.', 'logy' ); ?>
				</p>
			</div>
		<?php endif; ?>
